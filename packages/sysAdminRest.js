...
exports.deleteMany = (modelName, customFilter = {}) => {
  const Model = model(modelName);
  /**
   ...
   * @param {Function} next Go to the next middleware
   */
  return async function deleteMany(req, res, next) {
    const { ids = [] } = req.body;
    if (!ids || !Array.isArray(ids) || ids.length === 0) {
      return res.status(400).json({
        message: req.t('MISSING_IDS_TO_DELETE'),
      });
    }

    const objectIds = ids.filter((id) => Types.ObjectId.isValid(id)).map((id) => Types.ObjectId(id));

    try {
      const result = await Model.deleteMany({ _id: { $in: objectIds }, ...customFilter });
 ...
 * @param {Function} next Go to the next middleware
 */
exports.updateOne = async function updateOne(req, res, next, passToNext=false) {
...
exports.getById = (modelName) => {
  ...
  return async (req, res, next, id) => {
    try {
      ...
      try {
        req.$query = Model.findById(id).select(
          $select
            .split(',')
            .map((attr) => attr.trim())
            .filter(Boolean)
