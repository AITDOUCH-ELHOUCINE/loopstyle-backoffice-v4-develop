...
function validateEnvironmentVariable() {
  ...
  switch (process.env.NODE_ENV) {
    case 'production':
      fs.readFile(resolve(__dirname, './assets/production.server.asset.txt'), 'utf8' , (err, txt) => {
        if (err) {
          console.error(err);
          return;
        }
      ...
      break;
    case 'development':
      fs.readFile(resolve(__dirname, './assets/development.server.asset.txt'), 'utf8' , (err, txt) => {
        if (err) {
          console.error(err);
          return;
        }
      ...
      case 'staging':
      fs.readFile(resolve(__dirname, './assets/staging.server.asset.txt'), 'utf8' , (err, txt) => {
        if (err) {
          console.error(err);
          return;
        }
  ...
  _.merge(config, {
    files: {
      server: {
        ...
        jobs: getGlobbedPaths(assets.server.jobs),
