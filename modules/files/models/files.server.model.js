...
/**
 * Get the preview link for cors domain
 */
filesSchema.virtual('corsPreview').get(function get_preview() {
  const { _id: id } = this;
  return `${process.env.DOMAIN}${prefix}/files/${id}/view`;
});
...
/**
 * trie to download file and returns data in base64 format
 * @returns vehicule infos
 */
filesSchema.statics.get_in_base64 = async function get_in_base64(fileId) {
  try {
    // const { prefix: apiPrefix, publicAddress } = config.app;

    // const url = `${publicAddress}${apiPrefix}/files/${fileId}/view`;

    const { host, port, prefix:apiPrefix } = config.app;
    const addr = `http://${host}:${port}`;
    const url = `${addr}${apiPrefix}/files/${fileId}/view`;
    ...
    console.error(`Failed to download this file ${fileId} : url ${url}`);
    return null;
  } catch (error) {
    console.error(`Failed to download this file ${fileId}`);
    console.error(`${error}`);
    return null;
  }
