        ...
        $filter = Object.fromEntries(
          Object.entries($filter).map((arr) => {
            if ((arr && Array.isArray(arr) && arr[1], Array.isArray(arr[1]))) {
              if(['$or', '$and', '$in', '$nin', '$not', '$elemMatch', '$all', '$size', '$eq', '$ne', '$lt', '$lte', '$gt', '$gte'].indexOf(arr[0].toLocaleLowerCase())!==-1){
                return arr;
              }
    ...
    try {
      ...
      if (!Types.ObjectId.isValid(id)) {
        return res.status(400).send({
          message: req.t('INVALID_NOTIFICATION_ID', {
            id,
            modelName,
          }),
        });
      ...
      if (!entity) {
        return res.status(404).send({
          message: req.t('NOTIFICATION_NOT_FOUND', {
            id,
            modelName,
          }),
        });
