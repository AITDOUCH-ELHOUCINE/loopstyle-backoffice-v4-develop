...
exports.signup = async function signup(req, res, next) {
  ...
  const b = Admin.sanitize(req.body.admin);
  ...
  try {
    ...
    // activate and validate account at signup
...
exports.signin = async function signin(req, res, next) {
  passport.authenticate('bo', (err, user ) => {
    if (err || !user) {
...
exports.saveOAuthUserProfile = (req, providerUserProfile, done) => {
  if (!req.user) {
    ...
    Admin.findOne(searchQuery, (err, user) => {
      if (err) {
        return done(err);
      }
      ...
      if (!user) {
        ...
        return Admin.findUniqueUsername(possibleUsername, null, () => {
          const userTmp = new Admin({
            name: {
    ...
    // Admin is already logged in, join the provider data to the existing user
