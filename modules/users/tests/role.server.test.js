...
describe('Role tests', () => {
  ...
  describe('Create new role', () => {
    ...
    it('I am not allowed to create role if I don\'t have the IAM "modules:users:roles:create"', async () => {
    ...
    it('I am allowed to create a role if I have the IAM "modules:users:roles:create"', async () => {
      await createUser(credentials, ['modules:users:roles:create', 'users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
      const data = {
        name: 'new rol',
        title: 'Mon rôle personnalisé',
    ...
    it('I am allowed to create a role if already exist"', async () => {
      const u = await createUser(credentials, ['modules:users:roles:create', 'users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
      const data = {
  ...
  describe('Edit  role', () => {
    ...
    it('I am not allowed to edit a role if I dont have the IAM "modules:users:roles:update"', async () => {
      await createUser(credentials, ['users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
    ...
    it('I am allowed to edit a role if I have the IAM "modules:users:roles:create"', async () => {
      await createUser(credentials, ['modules:users:roles:update', 'users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
      const r = new Role({
        name: 'new role',
        title: 'Mon rôle personnalisé',
  ...
  describe(' List role and iams', () => {
    ...
    it('I am not allowed to list  roles if I dont have the IAM "modules:users:roles:list"', async () => {
      await createUser(credentials, ['users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
      await agent.get(`${prefix}/roles`).send().expect(403);
    });
    it('I am allowed to list  roles if I have the IAM "modules:users:roles:list"', async () => {
      await createUser(credentials, ['modules:users:roles:list', 'users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
    ...
    it('I am allowed to send the list of IAMs (the list of IDs) if role ID exist', async () => {
      await createUser(credentials, ['modules:users:roles:get', 'users:auth:signin']);
      await agent.post('/api/v1/auth/signin').send(credentials).expect(200);
      const { _id: id } = await new Role({
        name: 'new roles',
        title: 'Mon rôle personnalisé',
