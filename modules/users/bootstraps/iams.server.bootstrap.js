...
async function seedUsers() {
  ...
  const users = config.seedUsers ? config.seedUsers : [];
  ...
  const promises = users.map(async (user) => {
    try {
      ...
      if (!u) {
        debug(`create new ${user.email}`);
        const new_user = new UserModel(user);
        return await new_user.save();
      }
    } catch (e) {
      debug('DB Error while getting the user', e);
    }
...
async function seedAdmins() {
  debug('seeding Admins');
  const AdminModel = model('Admin');
  const admins = config.seedAdmins ? config.seedAdmins : [];

  // debug(admins);

  const promises = admins.map(async (admin) => {
    try {
      const u = await AdminModel.findOne({
        email: admin.email,
        // roles:'admin',
      });

      if (!u) {
        debug(`create new ${admin.email}`);
        const new_admin = new AdminModel(admin);
        return await new_admin.save();
      }
    } catch (e) {
      debug('DB Error while getting the admin', e);
    }
